...
class BOTrack(STrack):
    ...
    def is_jittering(track, 
                    velocity_thresh: float = 5.0,
                    position_jump_thresh: float = 50.0,
                    area_ratio_thresh: float = 2.5) -> bool:
        """
        Jittering 감지 함수. 박스의 이동 속도나 위치 변화가 비정상적으로 크면 True 반환.

        Args:
            track (BOTrack): 현재 추적 중인 트랙
            velocity_thresh (float): 속도 norm이 이 이상이면 튐으로 간주
            position_jump_thresh (float): 위치 변화가 이 거리 이상이면 튐
            area_ratio_thresh (float): 면적이 갑자기 커지거나 작아지면 튐
        ...
        if speed > velocity_thresh:
            return True

        # backup_mean이 존재할 경우 → 위치 변화량 확인
        if hasattr(track, 'backup_mean') and track.backup_mean is not None:
            prev_xy = track.backup_mean[:2]
            curr_xy = track.mean[:2]
            dist = np.linalg.norm(curr_xy - prev_xy)
            if dist > position_jump_thresh:
                return True

            # 면적 급변 여부 (a * h)
            prev_area = track.backup_mean[2] * track.backup_mean[3]
            curr_area = track.mean[2] * track.mean[3]
            ratio = max(curr_area / (prev_area + 1e-5), prev_area / (curr_area + 1e-5))
...
class BOTSORT(BYTETracker):
    ...
    def get_dists(self, tracks: List[BOTrack], detections: List[BOTrack]) -> np.ndarray:
        ...
        for t in tracks:
            if t.clingy:
                proximity_thresh *= 0.7  # reduce proximity threshold for clingy tracks
                appearance_thresh *= 0.7  # reduce appearance threshold for clingy tracks
                break # only need to adjust once for clingy tracks
        ...
        if self.args.with_reid and self.encoder is not None:
            # print("✅ [ReID 사용 중] 임베딩 거리 기반 매칭 수행 중...")
            emb_dists = matching.embedding_distance(tracks, detections) / 2.0
            emb_dists[emb_dists > (1 - appearance_thresh)] = 1.0
