...
class STrack(BaseTrack):
    ...
    def __init__(self, xywh: List[float], score: float, cls: Any):
        ...
        self.clingy = False  # whether this track is clingy
        ...
        self.snapshot = None  # for clingy tracks, stores the last known state
        self.total_conf = score  # total confidence for this track
    ...
    def update(self, new_track: "STrack", frame_id: int):
        ...
        self.frame_id = frame_id
        self.tracklet_len += 1
        self.total_conf += new_track.score  # accumulate confidence score
    ...
    def get_avg_conf(self) -> float:
        """
        Calculate the average confidence score of the track.

        Returns:
            float: The average confidence score of the track.
        """
        return self.total_conf / self.tracklet_len if self.tracklet_len > 0 else 0.0
...
class BYTETracker:
    ...
    def __init__(self, args, frame_rate: int = 30):
        ...
        self.lost_clingy_stracks = []  # type: List[STrack]
    ...
    def update(self, results, img: Optional[np.ndarray] = None, feats: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        for it in u_track:
            ...
            if track.state != TrackState.Lost:
                track.mark_lost()
                if track.clingy:
                    lost_clingy_stracks.append(track)
                else:
        ...
        for inew in u_detection:
            ...
            activated_stracks.append(track)
        # Step 4.5: Attempt to re-activate lost clingy tracks
        young_tracks = [t for t in activated_stracks if t.tracklet_len <= 20]
        dists = self.get_dists(self.lost_clingy_stracks, young_tracks)
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.args.match_thresh)
        for icling, iyoung in matches:
            cling = self.lost_clingy_stracks[icling]
            young = young_tracks[iyoung]
            cling.re_activate(young, self.frame_id, new_id=False)
            refind_stracks.append(cling) # re-activate clingy track with young track
            activated_stracks.remove(young)  # remove matched young track from activated_stracks
        # Step 4.75: Attempt to merge young tracks into tracked clingy tracks
        young_tracks = [t for t in activated_stracks if t.tracklet_len <= 20 and not t.clingy]
        clingy_tracked = [t for t in self.tracked_stracks if t.clingy]
        dists = self.get_dists(clingy_tracked, young_tracks)
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.args.match_thresh)
        for icling, iyoung in matches:
            cling = clingy_tracked[icling]
            young = young_tracks[iyoung]
            # 병합 로직
            print(f"🔁 Merging young track {young.track_id} into clingy track {cling.track_id}")
            cling.update(young, self.frame_id)
            activated_stracks.remove(young)  # remove matched young track from activated_stracks
        # Step 5: Update state
        ...
        for track in self.tracked_stracks:
            if track.tracklet_len >= 30 and track.get_avg_conf() >= 0.5 and not track.clingy:
                track.clingy = True
                track.snapshot = {
                    "feat": track.curr_feat.copy(),
                    "mean": track.mean.copy(),
                    "tlwh": track.tlwh.copy(),
                    "frame_id": self.frame_id
                }
                print(f"🧷 Cling track registered: ID {track.track_id}")
        self.lost_clingy_stracks = self.sub_stracks(self.lost_clingy_stracks, self.tracked_stracks)
        self.lost_clingy_stracks.extend(lost_clingy_stracks)
        if len(self.lost_clingy_stracks) > 10:
            self.lost_clingy_stracks = self.lost_clingy_stracks[-9:]  # clip clingy stracks to 10 maximum
